...
class ConfigParameterRequestSchema(BaseSchema):
    ...
    schema = Str(required=True, enum=PARAMETER_SCHEMAS,
                 example='yaml', description='Scheme.',
...
class ConfigRequestSchema(BaseSchema):
    ...
    actions = Nested(ConfigActionRequestSchema, many=True, unknown='INCLUDE',
                     description='List of actions.')
    parameters = Nested(ConfigParameterRequestSchema, many=True,
                        unknown='INCLUDE', description='List of parameters.')
    resources = Nested(ConfigResourceRequestSchema, many=True,
                       unknown='INCLUDE', description='List of resources.')
...
class ConfigResponseSchema(BaseSchema):
    ...
    timestamp = Date_Time(format=FORMAT, required=True,
                          description='Timestamp when the configuration is done.')  # noqa:E501
...
class ConfigActionResponseSchema(ConfigResponseSchema):
    ...
    return_code = Integer(required=True, example=0,
                          description='Exit code of the execution (0: no error).')  # noqa:E501

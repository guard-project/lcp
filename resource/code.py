...
class CodeResource(BaseResource):
    ...
    def on_post(self, req, resp, id=None):
        ...
        resp_data, valid = CodeRequestSchema(
            ...
            if len(req_data_wrap) > 0:
                for data in req_data_wrap:
                    id, code, interface, metrics = item_getter(
                        "id", "code", "interface", "metrics"
                    )(data)
                    if all([id, code, interface]):
                        if is_list(code):
                            code = "\n".join(code)
                        pc = self.polycube.create(
                            cube=id,
                            code=code,
                            interface=interface,
                            metrics=metrics,
                ...
                NoContentResponse(
                    MSG_NO_CONTENT.format("create"), request=req_data
                ).apply(resp)
        else:
            resp_data.apply(resp)
    ...
    def on_put(self, req, resp, id=None):
        req_data = req.media or {}
        resp_data, valid = CodeRequestSchema(
            many=is_list(req_data), partial=True, method=HTTPMethod.PUT
        ).validate(data=req.media, id=id)
        if valid:
            req_data_wrap = wrap(req_data)
            if len(req_data_wrap) > 0:
                ...
                NoContentResponse(
                    MSG_NO_CONTENT.format("update"), request=req_data
                ).apply(resp)
        else:
            resp_data.apply(resp)
    ...
    def on_delete(self, req, resp, id=None):
        req_data = req.media or {}
        resp_data, valid = CodeRequestSchema(
            many=is_list(req_data), partial=True, method=HTTPMethod.DELETE
        ).validate(data=req.media, id=id)
        if valid:
            req_data_wrap = wrap(req_data)
            if len(req_data_wrap) > 0:
                ...
                NoContentResponse(
                    MSG_NO_CONTENT.format("delete"), request=req_data
                ).apply(resp)
        else:
            resp_data.apply(resp)

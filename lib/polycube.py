...
class Polycube:
    def __init__(self):
        ...
        try:
            resp_req = get_req(
                self.endpoint, timeout=ArgReader.db.polycube_timeout
            )
            self.__manager(resp_req)
        except (ConnectionError, HTTP_Error) as e:
            self.log.exception(
                f"""Connection with polycube at
                                   {self.host}:{self.port} not possible""",
                e,
            )
    ...
    def get(self, cube):
        ...
        try:
            resp_req = get_req(
                f"{self.endpoint}/dynmon/{cube}", timeout=self.timeout
            )
            self.__manager(resp_req)
    ...
    def delete(self, cube):
        ...
        try:
            ...
            self.__manager(resp_req)
            return dict(
                status="deleted", data=data, **self.__from_resp(resp_req)
            )
        except Exception as e:
            self.log.exception(f"Cube {cube} not deleted", e)
            return dict(
                error=True, data=data, **self.__from_resp(resp_req, error=True)
            )
    ...
    def __detach(self, cube, interface):
        return self.__extracted_from___attach_2(
            "/detach", cube, interface, "detached"
        )

    def __attach(self, cube, interface):
        return self.__extracted_from___attach_2(
            "/attach", cube, interface, "attached"
        )
    ...
    def __manager(self, resp_req):
        try:
            ...
            raise HTTP_Service_Unavailable(
                title="Connection error", description=_msg
            )
        except Timeout as e:
            ...
            raise HTTP_Gateway_Timeout(
                title="Polycube Unavailable", description=_msg
            )
